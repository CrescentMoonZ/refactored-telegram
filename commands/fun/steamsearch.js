...
class SteamSearchCommand extends Command {
    constructor(client) {
        super(client, {
            name: 'steamsearch',
            group: 'fun',
            aliases: ['game'],
            memberName: 'steamsearch',
            description: 'Look up queries of games on steam!',
            clientPermissions: ['EMBED_LINKS'],
            examples: ['!steamsearch <game>'],
            args: [{
                key: 'query',
                prompt: 'Please provide me a game to search for :v',
                type: 'string',
    ...
    async run(message, args) {
        ...
        if(!search.body.items.length) return msg.say(`No results found for **${query}**.. Sorry!`);
        ...
        const current = data.price_overview ? `$${data.price_overview / 100}` : 'Free';
        const original = data.price_overview ? `$${data.price_overview / 100}` : 'Free';
...
module.exports = SteamSearchCommand;
*/